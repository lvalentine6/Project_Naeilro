...
function report_cmodal(e){
	$("#report_cmember").val($(e).data('member'))
	$("#report_ccomment").val($(e).data('comment'))
	$("#report_cno").val($(e).data('report'))
	$("#report_creason").val($('#report_reason_'+$(e).data('report')).text())
	$("#report_ccontent").val($(e).data('content'))
}
...
function delete_ccomment(e){
	if (!window.confirm("게시글을 삭제합니다") ){ 
		e.preventDefault()
	}
	let commentNo = $("#report_ccomment").val()
	let reportNo = $("#report_cno").val()
	let memberNo = $("#report_cmember").val()
	let reportDay = $("#report_cday").val()
	let reportReason = $("#report_creason").val()
	let reportContent = $("#report_ccontent").val()
...
<button type="button" class="btn btn-danger btn-sm storyNo_delete_btn" data-toggle="modal" data-target="#report_cmodal" onclick="report_cmodal(this)" data-content="{{comment}}"  data-comment="{{commentNo}}" data-report="{{reportNo}}" data-member="{{memberNo}}">게시글 삭제</button>
</div>
</script>
...
<div class="container-lg">
	...
	<div class="row">
		...
		<c:forEach items="${reportPList}" var="listItem">
...
<div class="container-lg mt-5">
	...
	<div class="row">
		<div class="accordion col-lg-10 offset-lg-1">
		<c:forEach items="${reportCList}" var="listItem">
		  <div class="card report_container_${listItem.reportNo}">
		    <div class="card-header row" >
		    	<div class="col-2 d-flex align-items-center">${listItem.no}</div>
		    	...
		    	<div class="col-2 d-flex align-items-center text-nowrap justify-content-center">
		    		<button class="btn btn-outline-primary  collapsed detail_cbtn" type="button" data-toggle="collapse" data-report="${listItem.reportNo}" data-target="#item_${listItem.reportNo}">
		          		자세히
		        	</button>
		    	</div>
		    	<div class="col-2 d-flex align-items-center text-nowrap justify-content-center">
...
<div class="modal fade" id="report_cmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title text-danger" id="exampleModalScrollableTitle">게시글 삭제</h5>
      ...
      <div class="modal-body report-val">
       ...
       <div class="row">
    		<div class="col-6"><a href="#none">유저 정지</a></div>
    		<div class="input-group mb-3 col-6">
			  <input type="number" min="0" class="form-control" value="0" id="report_cday">
			  <div class="input-group-append">
			    <span class="input-group-text" >일</span>
			  </div>
			</div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary r-c-btn" data-dismiss="modal">닫기</button>
        <button type="button" class="btn btn-danger r-c-btn" data-dismiss="modal" onclick="delete_ccomment(this)">게시글 삭제</button>
      </div>
    </div>
  </div>
</div>

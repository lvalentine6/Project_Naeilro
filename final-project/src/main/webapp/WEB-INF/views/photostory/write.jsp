...
<script>
	...
	$(function () {
		...
		$('.submit_btn').click(function (e) {
			...
			$('#photostoryContent').val(
				document.querySelector('mini-editor')._children.outp.innerHTML
			);
			let tag = document
				.querySelector('mini-editor')
				._children.outp.querySelectorAll('.hashtag');
			for (let i = 0; i < tag.length; i++) {
				$('.photostory_form').append(
					$('<input/>', { type: 'hidden', name: 'hashtag', value: tag[i].textContent })
				);
			}
		});
	});
	...
	function handleImgFileSelect(e) {
		...
		filesArr.forEach(function (f) {
			...
			reader.onload = function (e) {
				var html =
					'<div class="d-inline-block position-relative story-photo prev_img" id="img_id_' +
					index +
					'" style="width: 24%;height:' +
					$('.upload_img').width() +
					'px"> <img class="upload_img story-photo h-100 add_img" src="' +
					e.target.result +
					'"> <i class="fas fa-times text-danger position-absolute" onclick="deleteImageAction(' +
					index +
					')" style="right:4%; top:4%; font-size: 1rem"></i> </div>';
	...
	<div class="container-lg">
		<div class="row">
			<div class="jumbotron col-lg-6 offset-lg-3">
				<h3 class="display-5" id="text">스토리 작성</h3>
			</div>
			<div class="col-lg-6 offset-lg-3 text-center">
				<form
					action="write"
					method="post"
					class="photostory_form encrypt-form"
				...
				>
					...
					<div class="form-row mb-3 row">
						<div class="col text-left imgs_wrap">
							<label for="photostoryPhoto" style="width: 24%">
								<img
					...
					<div class="form-row mb-3">
						<label for="photostoryContent" class="w-100 text-left">스토리 내용</label>
						<mini-editor placeholder="내용을 입력하세요 (#해시태그)"></mini-editor>
						<small id="emailHelp" class="form-text text-muted"
							>스토리 내용을 작성해주세요.</small
						>
						<input type="hidden" name="photostoryContent" id="photostoryContent" />
					</div>

					<div class="form-row mb-5 justify-content-around">
						<button class="btn btn-primary submit_btn btn-block" type="submit">
							작성
						</button>
						<button class="btn btn-secondary cancel-btn btn-block" type="button">
							취소
						</button>

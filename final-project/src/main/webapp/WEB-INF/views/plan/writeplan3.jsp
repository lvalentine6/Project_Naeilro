			...
			$.ajax({
				...
				success:function(resp){
					for(var i=0; i < resp.length; i++){
						...
						template = template.replace("{{dailyNo}}", resp[i].dailyNo);
						template = template.replace("{{dailyOrder}}", resp[i].dailyOrder);
						$("#daily-list-confirm").append(template);
						console.log("하루계획표 " + resp[i].dailyOrder +"일차 출력 완료");
					}
					
					// 장소 리스트 출력
					$("#open-place-list-button").click(function(){
						console.log("장소 리스트 출력 시작");
					});
				},
				error:function(){
					console.log("하루계획표 출력 실패");
				}
			});
		}
		
		// DB : 등록 정보 리스트 출력 :  장소(하루계획표 기준)
		function placeListService(){
			$.ajax({
				url:"${pageContext.request.contextPath}/plan/data/placeListService",
				type:"get",
				dataType:"json",
				data:{
					dailyNo: $("input[name=dailyNo]").val()
				},
				success:function(resp){
					for(var i =0; i <resp.length; i++){
						var template = $("#list-place-template").html();
						template = template.replace("{{transfer}}", resp[i].dailyplanTransfer);
						template = template.replace("{{placeOrder}}", resp[i].dailyplanPlaceOrder);
						template = template.replace("{{placeLatitude}}", resp[i].placeLatitude);
						template = template.replace("{{placeLongitude}}", resp[i].placeLongitude);
						// 등록 장소 명 : API 통해 가져와보기
						$("#place-list-confirm").append(template);
						
						console.log("장소 리스트 출력 완료");
					}
				},
				error:function(){
					console.log("장소 리스트 출력 실패");
				}
			});
		}
...
<script type="text/template" id="list-daily-template">
	<!-- 테스트용 : 하루계획표 리스트 -->
	<div class="list-daily" style="border-top: 1px solid">
		<div class="row-list-daily">
			<label>계획표 번호</label>
			<input type="text" name="dailyNo" value={{dailyNo}} class="row-dailyNo">
		</div>
		<div class="row-list-daily">
			<label>계획표 순서</label>
			<input type="text" name="dailyOrder" value={{dailyOrder}} class="row-dailyOrder">
		</div>
		<div class="row-list-daily">
			<button id="open-place-list-button">장소 보기</button>
		</div>
	</div>
	<!-- 테스트용 : 하루계획표 리스트 -->
</script>
...
<body>
	...
	<div id="container" style="width: 100%; height: 800px; overflow: hidden"> 
		...
		<div style="width: 20%; height: 800px; border: 1px solid;float: left" id="confirm"> 
			...
			<div id="plan-select-confirm">
				<form id="place-select-form">
				<div><h4>개발영역 : 계획조회 전송</h4></div>
				<!-- 하루계획표 번호 -->
				<input type="text" name="dailyNo" required readonly placeholder="하루계획표 번호">
				<!-- 하루계획표 번호 -->
				</form>
			</div>
			<!-- 개발자 컨테이너 : 등록 -->

	...
	$(function(){
		...
		function dailyTemplate(dailyStayDate){
			...
			for(var i=0; i < dailyStayDate; i++){
				...
				if (placeRegionList[i] != null) {
					template = template.replace("{placeRegion}", placeRegionList[i]);
				} else {
					template = template.replace("{placeRegion}", "지역 :");
				}
		...
		function setMapBounds(placeName, placeType, keyword){
			...
			function displayMarker(place, placeType, placeName) {
			    ...
			    kakao.maps.event.addListener(marker, 'click', function(){
			        ...
			        /* 뷰 */
			        dailyIndex = $('#daily-index').val(); // 하루계획표 인덱스 선택자
					placeIndex = $(".list-daily").eq(dailyIndex-1).find(".list-dailyplan").last().data("index"); // 장소 선택자
		...
		<c:forEach var="plan" items="${planList}">
			placeRegionArr.push('${plan.placeRegion}');
		</c:forEach>

		var placeRegionList = [];
		for (var i = 0; i < dailyplanCountList.length; i++) {
			placeRegionList.push(placeRegionArr[i * dailyplanCountList[i]]);
		}
			...
			for (var i = 0; i < dailyStayDate; i++) {
				...
				<c:forEach var="plan" items="${planList}">
					...
					if (dailyNoArr[i] == ${plan.dailyNo}) {
			        	...
			        	dataTemplate = dataTemplate.replace("{exist-result}", "true");
						...
						// 데이터 전송 : 전송 값
						dataTemplate = dataTemplate.replace("{dailyplanPlaceOrder}", placeIndex);
						dataTemplate = dataTemplate.replace("{dailyplanTransfer}", '${plan.dailyplanTransfer}');

						dataTemplate = dataTemplate.replace("{placeLatitude}", '${plan.placeLatitude}');
						dataTemplate = dataTemplate.replace("{placeLongitude}", '${plan.placeLongitude}');
						dataTemplate = dataTemplate.replace("{placeName}", '${plan.placeName}');
						dataTemplate = dataTemplate.replace("{placeType}", '${plan.placeType}');
						dataTemplate = dataTemplate.replace("{placeRegion}", '${plan.placeRegion}');
						$("#plan-insert-container").append(dataTemplate);
						placeIndex++;
					}
...
<script type="text/template" id="place-dailyplan-insert-template">
	<!-- 비동기 처리 : 장소 & 장소계획 생성(N회) -->
		<div class="planList-place" data-index={placeList-index-data}>
			...
			<div class="exist-result" data-exist="{exist-result}"></div>
	...
	<ul class="list-daily card hoverable z-depth-3 collapsible" data-index="{index}">
		...
		<li class="center active">
			<div class="list-daily-placeName">
				<span id="daily-input-name">{placeRegion}</span>
			</div>
				...
				<div class="col-xs-6 col-md-3">
					...
					<div id="planner-insert-confirm" >
						<ul class="card hoverable z-depth-3 collapsible">
							<h6 class="center">
								<b><span>계획표 수정</span></b>
							</h6>
							<li class="center active">
								<div class="daycount-main1">
									<input class="center" type="text" id="planner-name" value="${planList.get(0).plannerName}" placeholder="계획표 이름">
								</div>
